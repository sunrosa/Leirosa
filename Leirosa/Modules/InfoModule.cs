namespace Leirosa.Modules
{
    public class InfoModule : Discord.Commands.ModuleBase<Discord.Commands.SocketCommandContext>
    {
        ...
        public async Task RuntimeAsync()
        {
            ...
            log.Debug("Replying...");
            await ReplyAsync($"Running {Program.Config.BotName} {Program.Version}{(commitSha != "" ? $" {commitSha[0..7]}" : "")} on .NET {Environment.Version} on {System.Net.Dns.GetHostName()} with build configuration {buildConfiguration}.");
        }
